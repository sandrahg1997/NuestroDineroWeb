-- OurMoney · Ejecutar completo en Supabase > SQL Editor
...
-- Procesa los recurrentes vencidos de UN hogar (lo llama la app al abrirla, con
-- la sesión del usuario). El `guard` evita que una next_date muy antigua genere
...
-- Procesa los recurrentes vencidos de TODOS los hogares. La ejecuta el cron de
-- Vercel (/api/cron/recurring) con la service_role key, sin auth.uid(): por eso
-- el movimiento generado se atribuye a la persona propietaria del hogar.
